...
int main (int argc, char *argv[])
{
    ...
    cmd.addGroup("options:", LONGCOL, SHORTCOL);
     ...
     cmd.addOption("--delete-by-filename", "-dbf", 1, "[c]ommand: string",
    ...
    {
        if (cmd.findOption("--delete-by-filename"))
        {
            app.checkValue(cmd.getValue(opt_delFilename));
        }
    ...
    if (cond.good())
    {
        ...
        if (opt_delFilename != "")
        {
            IdxRecord idxRec;
            int i = -1;
            COUT << "opt_delFilename=" << opt_delFilename << OFendl;
            hdl.DB_IdxInitLoop(&i);
            while (1) {
                if (hdl.DB_IdxGetNext(&i, &idxRec) != EC_Normal)
                    break;
                //hdl.DB_IdxRemove(0);
